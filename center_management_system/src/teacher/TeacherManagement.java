...
public class TeacherManagement extends Management{
	...
	protected void printMenu() {
		System.out.println();
		System.out.println("①강사등록 ②전체조회 ③강사검색 ⑨뒤로가기");
	}
	...
	private void selectOne(List<Teacher> list) {
		...
		System.out.println("[No.] [강의번호] [강좌명] [장소] [요일] [정원] [등록인원]");
		List<Classes> Classlist =  classDAO.selectAll(teacherId);
		int num = 0;
		for(Classes classes : Classlist) {
			System.out.print("  " + ++num + "    ");
			System.out.println(classes.getClassId() + "    " + String.format("%-5s", classes.getClassName()) + " " + classes.getClassPlace() + " " + classes.getClassDay()+ "   " + classes.getClassCapacity() + "      " + classes.getEnrollment());
		}
